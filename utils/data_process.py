...
class DataProcessor(object):
	...
	def process_outputs(self,out,input_img_shape,features_shape=None,patch_shape=None,group_size=1):
		...
		if patch_shape is not None:
			if len(features_shape) > 2:
				...
				feat_h,feat_w = features_shape
			
			patch_shape = OmegaConf.to_container(self.cfg_dataset.patch_shape)
			patch_shape = patch_shape if type(patch_shape) is list else (patch_shape,patch_shape)

			out_reshape = rearrange(out,'(h w) b c ph pw -> b c (h ph) (w pw)',h=feat_h//patch_shape[0],w=feat_w//patch_shape[1],b=group_size,c=3)
	...
	def normalize_coordinates(self,coordinates,data_shape,normalize_range=[-1,1]):
		...
		if self.cfg_dataset.coord_separate_norm:
			...
			#max_dim = max(H,W)
			norm = torch.tensor([H,W])
			coordinates = coordinates / (norm - 1) #(max_dim - 1) 
	...
	def get_features(self,data,patch_shape=None):
		...
		data_shape = torch.tensor(data.shape).tolist()
		if data.ndim == 3:
			C,H,W = data.shape
			N = 1
		else:
		...
		if patch_shape is not None:
			...
			data_shape = torch.tensor(data_shape).tolist()
			if len(data_shape) == 2:
				data_shape = [C] + data_shape
...
if __name__ == '__main__':
	...
	print("unit tests for data_process.py")
