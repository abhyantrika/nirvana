...
class lth:
    def __init__(self, model, keep_percentage, n_rounds, late_reset_iter):
        ...
        print(self.get_lth_stats(full_model=True))
    ...
    def create_mask(self, model):
        ...
        for name, param in model.named_parameters():
            if ('weight' in name):# and ('stem' in name) or ('last_layer' in name):
                print('pruning,', name)
    ...
    def generate_new_mask(self, model, pruning_round):
        ...
        """
            Code optimized for one-shot pruning.
        """
    ...
    def get_lth_stats(self,full_model=False):

        """
            If full_model is True, then we calculate the pruning stats for the full model.
            Else, we calculate the pruning stats only for layers specified to be pruned.
        """
        ...
        for name, param in self.model.named_parameters():
            if (name in self.mask) or full_model:
                # print(name)
...
"""
Eg:
    lth_pruner = lth(model, args.keep_percentage, args.num_rounds, args.late_reset_epoch)
"""