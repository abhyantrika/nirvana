...
if __name__ == '__main__':
    #dummy example of encoder. 
    encoder = GridEncoder(input_dim=2, num_levels=1, level_dim=2, per_level_scale=2, \
                          base_resolution=16, log2_hashmap_size=19, desired_resolution=None,\
                            gridtype='hash', align_corners=False, interpolation='linear')
    encoder = encoder.cuda()
    print(encoder)
    print(encoder.embeddings.shape)
    #forward with dummy input. 
    inputs = torch.randn((2000,2)).cuda()
