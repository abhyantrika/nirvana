...
torchrun --nproc_per_node=2  main.py  data.data_path=/fs/cfar-projects/frequency_stuff/vid_inr/frames/jockey_1080/ \
logging.checkpoint.logdir=output/temp_jockey/ \
trainer.num_iters=2000 \
trainer.num_iters_first=2000 \
...
trainer.lr=5e-4 data.max_frames=6 \
...
logging.checkpoint.skip_save_model=True trainer.distributed=True
